...
public class BeanScanner extends ElementScanner6<Void, BeanDescription> {
    ...
    public Void visitExecutable(ExecutableElement e, BeanDescription p) {
        ...
        if (e.getKind() == ElementKind.CONSTRUCTOR) {
            ...
            for (VariableElement ve : ee.getParameters()) {
                cd.addParam(new ConstructorParamDescription(cd,
        ...
        if (e.getKind() == ElementKind.METHOD) {
            ...
            DisplayPropertyPrism displayPropertyPrism = DisplayPropertyPrism
                    .getInstanceOn(e);
            if (displayPropertyPrism != null) {
                ...
                String propertyClassName = processingEnvironment.getTypeUtils()
                        .asElement(e.getReturnType()).getSimpleName()
                        .toString();
                if (simpleName != null && simpleName.length() >= 4
                        && (simpleName.startsWith("set")
                                || simpleName.startsWith("get"))) {
                    String propertyName = simpleName.substring(3, 4)
                            .toLowerCase() + simpleName.substring(4);
    ...
    private void createPropertyDescriptions(BeanDescription p,
            DisplayPropertyPrism displayPropertyPrism, String propertyClassName,
            String propertyName) {

        PropertyDescription pd = new PropertyDescription(p, propertyName,
                propertyClassName);
    ...
    private void addProfiles(PropertyDescription pd,
            DisplayPropertyPrism displayPropertyPrism) {
        for (DisplayPropertySettingPrism dpsp : displayPropertyPrism
                .profileSettings()) {
            DisplayPropertyProfileDescription dpp = pd
                    .getDisplayPropertyProfile(dpsp.profileName());
