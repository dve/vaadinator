...
public class TestCompile {
	...
	public void testEnumeration() {
		...
		Compilation compilation = javac().withProcessors(mappingProcessor)
				.compile(
				JavaFileObjects.forResource("de/akquinet/engineering/vaadinator/example/address/model/Anreden.java"));
	...
	public void test() {
		MappingProcessor mappingProcessor = new MappingProcessor();
		Compilation compilation = javac().withProcessors(mappingProcessor).compile(
				JavaFileObjects.forResource("de/akquinet/engineering/vaadinator/example/address/model/Address.java"));
		assertThat(compilation).succeeded();
		...
		assertEquals("de.akquinet.engineering.vaadinator.example.address.ui.std.presenter",
		...
		assertEquals("\"anrede\", \"vorname\", \"nachname\", \"email\", \"geburtsdatum\"",
				desc.getDisplayProfile("std").getSortedPropertiesInProfileForDetailStr());
		...
		assertNotNull(desc.getProperty("anrede")
				.getEnumClass(Collections.singletonList(new BeanDescription("Anreden", true))));
		...
		assertEquals("getAnrede",
				desc.getProperty("anrede").getMapPropertyProfile("full").getTargetPropertyGetterName());
		assertEquals("setAnrede",
				desc.getProperty("anrede").getMapPropertyProfile("full").getTargetPropertySetterName());
		...
		assertEquals(FieldType.DATEPICKER,
				desc.getProperty("geburtsdatum").getDisplayPropertyProfile("std").getFieldType());
